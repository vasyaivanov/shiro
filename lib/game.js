...
//check answer
function testIfAnswerIsCorrect(teamAnswer, correctAnswer) {
  ...
  if(correctAnswer && correctAnswer.startsWith("/")) {
    ...
    // removes first '/'
    try {
      isAnswerCorrect = (new RegExp(correctAnswer.substring(1))).test(teamAnswer);
    } catch (err) {
...
Game.prototype.addAnswer = function Game_addAnswer(data, callback)
{
  ...
  _game.getQuestion({index: _game.state["current_question"]}, function Game_getQuestionAnswerCallback(err, question)
  {
      ...
      if(!err && question && teamData && teamData.answers){
        ...
        var isAnswerCorrect = testIfAnswerIsCorrect(teamAnswer,question.answer);
