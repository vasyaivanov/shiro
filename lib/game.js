...
Game.prototype.init = function Game_init(callback)
{
  ...
  level(this._storage, {keyEncoding: 'utf8', valueEncoding: 'json'}, function(err, db)
  {
    ...
    }, function(err, res)
    {
      ...
      // keep state sane
      if ((_game.state['current_question']  != false) && (_game.question[_game.state['current_question']] != false))
      {
...
Game.prototype.evalAnswer = function Game_evalAnswer(data, callback)
{
  ...
  this.save('team', teamData.login, teamData, function(err)
  {
    ...
    return callback(null, teamData, _game);
  });
}
...
function testIfAnswerIsCorrect(teamAnswer, correctAnswer, correctRegex) {
  ...
  if(correctRegex) {
    ...
    //Simple answer
    //isAnswerCorrect = teamAnswer.includes(correctAnswer) || correctAnswer.includes(teamAnswer);
    //isAnswerCorrect = (correctAnswer.localeCompare(teamAnswer) == 0);
    isAnswerCorrect = teamAnswer.includes(correctAnswer);
  }
...
Game.prototype.addAnswer = function Game_addAnswer(data, callback)
{
  ...
  _game.getQuestion({index: _game.state["current_question"]}, function Game_getQuestionAnswerCallback(err, question)
  {
      ...
      if(!err && question && teamData && teamData.answers){
        ...
        _game.evalAnswer(data,function(err,somedata,game){
            // save
          ...
          // console.log("JD: error. code:" + err.code + " message:"+err.message);
        });
      }
  });
}
...
Game.prototype.setQuestion = function Game_setQuestion(question, callback)
{
  ...
  if ((question.index != false) && (this.question[question.index] == false))
  {
  ...
  if ((this.state['current_question'] != false ) && (this.question[this.state['current_question']] != false ) && this.question[this.state['current_question']].answer_shown)
  {
  ...
  questionData = !(question.index === false) ? this.question[question.index] : {index: false};
...
Game.prototype.addQuestion = function Game_addQuestion(data, callback)
{
  ...
  // create team data
  questionData = {text: data.text, answer: data.answer, regex: data.regex};
...
Game.prototype._showQuestion = function Game__showQuestion(show)
{
  ...
  if (!show || ( this.state['current_question'] === false ) || (this.question[this.state['current_question']] === false ) )
  {
    displayData = false;
  }
  else
