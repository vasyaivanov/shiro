...
function calculateDollars(myTeamData, _game, questionNumber) 
{
...
Game.prototype.addAnswer = function Game_addAnswer(data, callback)
{
  ...
  // add answer
  teamData.answers[question] = {text: data.answer.text, time: diff, bonus: timeBonus, dollars: dollars, correct: null};
  ...
  _game.getQuestion({index: _game.state["current_question"]}, function Game_getQuestionAnswerCallback(err, question)
  {
      ...
      if(!err && question && teamData && teamData.answers){
        ...
        _game.evalAnswer(data,function(err,updatedTeams,game){
...
Game.prototype.addQuestion = function Game_addQuestion(data, callback)
{
  ...
  // create team data
  questionData = {prequestion: data.prequestion, text: data.text, answer: data.answer, regex: data.regex};
...
Game.prototype._resetAll = function Game_resetAll(callback)
{
  ...
  {
    this.team[teamKey] = _.merge(this.team[teamKey], {points: 0, time_bonus: 0, dollars: 0, adjustment: 0});
    this.team[teamKey].answers = {};
    pipeline.set(token[channel]+':'+teamKey, _game._convertToRedisString(this.team[teamKey]));
  }

  for (var stateKey in this.state)
  {
    this.state[stateKey] = false;
    pipeline.set(token[channel]+':'+stateKey, _game._convertToRedisString(false));
  }
  ...
  for (var questionKey in this.question)
  {
    ...
    pipeline.set(token[channel]+':'+questionKey, _game._convertToRedisString(this.question[questionKey]));
  }
