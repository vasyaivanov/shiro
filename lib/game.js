...
Game.prototype.init = function Game_init(callback)
{
  ...
  try{
      ...
      }, function(err, res)
      {
        ...
        {
          console.log("JD: before calling callback(null)");
          try{
            initCallback(null);
          } catch (err) {
...
function calculateDollars(teamData, _game, questionNumber) 
{
      ...
      if(teams){
        for (team in teams)
        {
          ...
          try {
            var answer = _game.team[team].answers[questionNumber]; 
      ...
      teamData.dollars = dollars + Number(teamData.dollars);
      return teamData.dollars;
}
